package net.cjsmc.progames.hungergames;
...
public class HungerGamesGame implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("hungergames")) {
            if (sender instanceof Player) {
                ...
                if (gameStarted) {
                    ...
                    if (!players.contains(player)) {
                        players.add(player);
                        scores.put(player, 0);
                        player.sendMessage("§aYou joined the Hunger Games!");
                ...
                sender.sendMessage("§cOnly players can start Hunger Games.");
            }
            return true;
        }
        return false;
    ...
    private void startGame() {
        gameStarted = true;
        Bukkit.broadcastMessage("§aHunger Games has started! May the odds be in your favor!");
    ...
    private void endGame() {
        ...
        if (winner != null) {
            Bukkit.broadcastMessage("§a" + winner.getName() + " has won the Hunger Games!");
        } else {
